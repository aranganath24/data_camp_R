...
install.packages(c("tidyverse", "fastDummies", "psych", "stargazer", "janitor", "gt"))
```
...
## Exporting summary statistics tables

The *stargazer* package also offers a convenient way to export a table of summary statistics. For example, the following exports the summary statistics for `qog_copy_selection_numeric` as a text file via `stargazer()`

```{r}
#| results: hide
# exports summary stats table as a text file with stargazer
stargazer(as.data.frame(qog_copy_selection_numeric), type="text", title="Descriptive Statistics", digits=1, out="outputs/summary_stats.txt")
```

If, instead, we want to export summary statistics as an html file, we can use the following:

```{r}
#| message: false
#| results: hide
# exports summary stats table as a text file with stargazer
stargazer(as.data.frame(qog_copy_selection_numeric), type="html", title="Descriptive Statistics", digits=1, out="outputs/summary_stats.html")
```

If we want to export a summary statistics table generated with `describe()` from the *psych* package, we can do so as a CSV file. For example, the code below exports `qog_copy_selection_numeric_summarystats2` as a CSV file. Before exporting, we'll convert the row names to an explicit columns, and then select a few columns of interest:

```{r}
# exports "qog_copy_selection_numeric_summarystats2" as CSV file after converting rownames to columns and selecting desired summary statistics for final output
qog_copy_selection_numeric_summarystats2 %>% 
  rownames_to_column(var="variable") %>% 
  dplyr::select(variable, n, mean, sd, min, max) %>% 
  write_csv("outputs/qog_numeric_summarystats2.csv")
```

If, instead, we want to export `qog_copy_selection_numeric_summarystats2` as a pdf file instead, we can use the `gt()` and `gtsave()` functions from the *gt* package. Note that this package defaults to scientific notation, so we use `fmt_number()` to specify the use of standard notation where numbers are rounded to two decimal places (except the "n" column, which are printed as integers):

```{r}
# exports "qog_copy_selection_numeric_summarystats2" as PDF table after converting rownames to columns, selecting desired summary statistics, and turning off scientific notation
qog_copy_selection_numeric_summarystats2 %>% 
  rownames_to_column(var="variable") %>% 
  dplyr::select(variable, n, mean, sd, min, max) %>% 
  gt() %>% 
  ...
  fmt_number(
    ...
    decimals = 2) %>% 
  gtsave("outputs/qog_numeric_summarystats2.pdf")
```
...
If you want to iteratively export multiple visualizations as separate files, we can put the objects we want to export in a list, and export them using the `walk()` function in conjunction with `ggsave()` (similar to what we did above, when exporting multiple CSV files). Below, we export `trade_southasia_ascending` and `tax_trade_scatter_line` as separate **.png** files to the outputs folder in our working directory.

```{r}
# creates list to hold visualization objects with inline labeling
viz_list<-list("trade_southasia_ascending"=trade_southasia_ascending, "tax_trade_scatter_line"= tax_trade_scatter_line)

# create file names
viz_filenames<-paste0("outputs/", names(viz_list), ".png")

# export files
walk2(.x=viz_filenames, .y=viz_list, .f=ggsave, width=12, height=5)
